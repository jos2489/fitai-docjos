...
export default function History({ state }) {
  ...
  if (sessions.length === 0) {
    return (
      <div className="fade">
        <div className="section-title">Storico allenamenti</div>
        <div className="card empty">
          <div className="big">📖</div>
          <div>Qui troverai tutti gli allenamenti completati, con carichi, volume e note.</div>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fade">
      <div className="section-title">Storico · {sessions.length} allenamenti</div>
      {sessions.map((s) => (
        <div key={s.dk} className="card" style={{ padding: 0, overflow: 'hidden' }}>
          <button className="hist-head" onClick={() => setOpen(open === s.dk ? null : s.dk)}>
            <div>
              ...
              <div style={{ color: 'var(--muted)', fontSize: 12, marginTop: 2 }}>{fmtDate(s.date)} · Sett. {s.week}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontWeight: 800, color: 'var(--accent-2)' }}>{fmt(s.tonnage)} kg</div>
              <div style={{ color: 'var(--muted)', fontSize: 12 }}>{s.setCount} serie</div>
            </div>
          </button>
          {open === s.dk && (
              ...
              {s.exLogs.map((e, i) => (
                <div key={i} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ color: 'var(--accent-2)', fontSize: 13, marginTop: 2 }}>
                    {e.log.length ? e.log.map((x) => `${x.weight || '–'}×${x.reps || '–'}`).join('   ') : '— non registrato'}
                  </div>
                </div>
              ))}
              {s.note && (
